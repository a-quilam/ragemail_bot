...
async def cmd_help(m: types.Message, db) -> None:
    ...
    if is_admin:
        help_text = (
            ...
            "• <code>/block слово [причина] [время_в_часах]</code> — заблокировать слово\n"
            "• <code>/unblock слово</code> — разблокировать слово\n"
            "• <code>/blocks</code> — показать все блокировки\n"
            "• <code>/cooldown псевдоним [время_в_часах] [причина]</code> — кулдаун по псевдониму\n"
            "• <code>/remove_cooldown user_id</code> — снять кулдаун\n"
            "• <code>/cooldowns</code> — показать все кулдауны\n\n"
