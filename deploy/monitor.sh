...
# Использование: ./monitor.sh [server_ip]
...
# Проверка параметров
if [ $# -lt 1 ]; then
    error "Использование: $0 <server_ip>"
...
ssh $SERVER_USER@$SERVER_IP "systemctl show telegram-bot --property=ActiveEnterTimestamp --value" 2>/dev/null || echo "Неизвестно"

# Использование ресурсов
echo ""
log "Использование ресурсов..."
echo "CPU и память:"
ssh $SERVER_USER@$SERVER_IP "ps aux | grep 'python run_bot.py' | grep -v grep" 2>/dev/null || echo "Процесс не найден"
