...
# Использование: ./deploy.sh [server_ip] [bot_token]
...
# Проверка параметров
if [ $# -lt 2 ]; then
    error "Использование: $0 <server_ip> <bot_token>"
    error "Пример: $0 192.168.1.100 your_bot_token_here"
    exit 1
fi
...
echo "  Статус бота:     ssh $BOT_USER@$SERVER_IP '$PROJECT_DIR/monitor_bot.sh'"
